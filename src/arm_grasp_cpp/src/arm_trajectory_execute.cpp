...
private:
    ...
    void trajectory_gen_and_pub()
    {
        ...
        std::vector<std::vector<double>> dmp_traj = this->readCSV(csv_file);
        ...
        if (dims != 3)
        {
            RCLCPP_ERROR(this->get_logger(), "CSV trajectory must have 3 rows for x,y,z.");
            return ;
        }
        ...
        RCLCPP_INFO(this->get_logger(), "Cartesian path computed: %f%%", fraction * 100.0);
        
        if (fraction < 0.90)
        {
            RCLCPP_WARN(this->get_logger(), "Trajectory not fully planned!");
            return ;
        }
